...
class Image:
    def __init__(self):
        self.data = []
        self.num = 0
        # set value in update_image_label
        self.label = []
        self.fn = ""
...
# 测试数据
def test():
    resultFile = open("./semeionsvmresult.txt", "w")
    recog = 0
    recog_correct = 0
    for img in gv.tests: 
        ...
        for mno in range(10):
            ...
            if predict(img) > 0:
                ...
                if mno == int(img.num):
                    recog_correct += 1
                    resultFile.write("true-%d" % int(mno))
                    resultFile.write("\n")
                    print 'true!!!!!!!!!!!!!!!!!!!!'
                else:
                    resultFile.write("false-%d" % int(mno))
                    resultFile.write("\n")
                break
            else:
                if mno == 9:
...
def save_models():
    ...
    for i in range(10):
        fn = open(basepath + str(i) + "_a.model", "w")
        for ai in gv.models[i].a:
            fn.write(str(ai))
            fn.write('\n')
        fn.close()
        fn = open(basepath + str(i) + "_b.model", "w")
        fn.write(str(gv.models[i].b))
        fn.close()

def load_models():
    for i in range(10):
        fn = open("./svmmodel/" + str(i) + "_a.model", "r")
        j = 0
        for line in fn:
            gv.models[i].a[j] = float(line)
            j += 1
        fn.close()
        fn = open("./svmmodel/" + str(i) + "_b.model", "r")
...
if __name__ == "__main__":
    ...
    if training == True:
        ...
        if iscached == False:
            gv.init_cache_kernel()
            with open("./kernelcache.pickle", "wb") as cachedata:
                cPickle.dump(gv.cache_kernel, cachedata)
        else:
            with open('./kernelcache.pickle', 'rb') as cacheData:
                gv.cache_kernel = cPickle.load(cacheData)
    gv.init_models()
