...
if __name__ == '__main__':
    ...
    if os.path.isfile("./mapmatrix.pickle"):
        with open("./mapmatrix.pickle", "rb") as cacheData:
            mapMatrix = cPickle.load(cacheData)
    else:
        with open('./mapmatrix.pickle', 'wb') as cacheData:
    ...
    resultfile = open("./pcaresult.txt", "w")
    ...
    for dir in dirs:
        ...
        for file in files:
            testsum += 1
            test = parse_image(source + dir + '/' + file)
            verify.updateInput(np.dot(mapMatrix, np.array(test)))
            result = verify.getNumber()
            if int(result) == int(dir[0]):
                correct += 1
            else:
                pass    
        resultfile.write("%d accuracy ---> %f\n" % (int(dir[0]), float(correct)/float(testsum)))
