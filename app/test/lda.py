...
class Verify:
    ...
    def getNumber(self, target):
        self.y = np.dot(target, self.mapvector)
        if self.flag == 1:
            if float(self.y) > float(self.sepline):
                return 3
            else:
                return 1
        else:
...
def ldadecision(target):
    cachePath = str(os.path.join(os.path.dirname(__file__), './mapcache.pickle').replace('\\', '/'))
    ...
    for i in xrange(len(mapcache)):
        ...
        verify = Verify(sepline, flag, mapvector)
        result = verify.getNumber(target)
        if int(result) == 3:
            return i
        else:
            pass
...
if __name__ == '__main__':
    imagePath = str(os.path.join(os.path.dirname(__file__), '../testsamples/cleantha.png').replace('\\', '/'))
    imagefilter = ImageFilter(imagePath)
